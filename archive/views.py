...
from archive.models import (Creator, Location, Production, WorkRecord,
                            DigitalObject, Festival,
                            FestivalOccurrence, HomePageInfo,
...
class CreatorsListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        """
            Make a container for all the object info -
            link to file + file info + creator id
        """
        objects_list = []
        ...
        alldos = DigitalObject.objects.filter(
            published=True,
            related_creator__isnull=False,
            files__isnull=False,
            digi_object_format=imagetype)
        length = len(alldos) - 1
        count = 0
        dos = []
...
class CreatorsAlphaListView(CreatorsListView):
    def get_queryset(self):
        if self.kwargs['alpha'] == '0':
            qset = Creator.objects.filter(
                published=True).filter(
                creator_name__iregex=r'^[0-9!@#$%^&*\(\)]'
                ).select_related().order_by('creator_name')
        else:
            qset = Creator.objects.filter(
                published=True
                ).filter(
                creator_name__istartswith=self.kwargs['alpha']
                ).select_related().order_by('creator_name')
        return qset
...
class CreatorDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        alldos = DigitalObject.objects.filter(
            published=True,
            related_creator=self.object,
            files__isnull=False,
            digi_object_format=imagetype).distinct()
        ...
        videos = DigitalObject.objects.filter(
            published=True,
            related_creator=self.object,
        ...
        audios = DigitalObject.objects.filter(
            published=True,
            related_creator=self.object,
            digi_object_format=audiotype).distinct()
        audio_list = []
        for audio in audios:
            item = {}
...
class ProductionsListView(ListView):
    queryset = Production.objects.filter(
        published=True).select_related().order_by('title')
    ...
    def get_context_data(self, **kwargs):
        ...
        alldos = DigitalObject.objects.filter(
            published=True,
            related_production__isnull=False,
            files__isnull=False,
            digi_object_format=imagetype)
        count = 0
        length = len(alldos) - 1
...
class ProductionsAlphaListView(ProductionsListView):
    def get_queryset(self):
        if self.kwargs['alpha'] == '0':
            qset = Production.objects.filter(
                published=True
                ).filter(
                title__iregex=r'^[0-9!@#$%^&*\(\)]'
            ...
            qset = Production.objects.filter(
                ...
                title__istartswith=self.kwargs['alpha']
                ).select_related().order_by('title')
        return qset

    def get_context_data(self, **kwargs):
        context = super(ProductionsAlphaListView,
...
class ProductionDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        alldos = DigitalObject.objects.filter(
            published=True,
            related_production=self.object,
            files__isnull=False,
            digi_object_format=imagetype).distinct()
...
class WorkRecordsAlphaListView(WorkRecordsListView):
    def get_queryset(self):
        if self.kwargs['alpha'] == '0':
            qset = WorkRecord.objects.filter(
                published=True
                ).filter(
                title__iregex=r'^[0-9!@#$%^&*\(\)]'
                ).select_related().order_by('title')
    ...
    def get_context_data(self, **kwargs):
        context = super(WorkRecordsAlphaListView,
                        self).get_context_data(**kwargs)
        if self.kwargs['alpha'] == '0':
            alpha = '#'
        else:
...
class VenuesListView(ListView):
    ...
    queryset = Location.objects.filter(
        ...
        productions__isnull=False
        ).select_related().distinct().order_by('title')
...
class VenueDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object.photo:
            context['venuephoto'] = default.backend.get_thumbnail(
                self.object.photo.files.all()[0].filepath,
                "100x100", crop="center")
        return context


class DigitalObjectsListView(ListView):
    queryset = DigitalObject.objects.filter(
        Q(published=True),
        Q(digi_object_format__title="Image", files__isnull=False) |
        Q(digi_object_format__title="Video recording",
          ready_to_stream=True)).distinct().select_related().order_by('title')
...
class DigitalObjectsVideosListView(ListView):
    ...
    queryset = DigitalObject.objects.filter(
        published=True, ready_to_stream=True,
...
class DigitalObjectsImagesListView(ListView):
    imagetype = DigitalObjectType.objects.get(title="Image")
    queryset = DigitalObject.objects.filter(
        published=True, files__isnull=False,
        digi_object_format=imagetype
        ).distinct().select_related().order_by('-creation_date')
    context_object_name = 'digital_objects'
    template_name = "archive/digitalobjects_list.html"
...
def phys_types_list(request):
    ...
    return render_to_response('archive/digitalobjecttypes_list.html',
                              context_dict,
...
class DigitalObjectsTypeListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DigitalObjectsTypeListView,
                        self).get_context_data(**kwargs)
        try:
            context['physobjtype'] = PhysicalObjectType.objects.get(
                slug__iexact=self.args[0]).title
        except:
            context['physobjtype'] = self.args[0]
        return context

    def get_queryset(self):
        type_name = get_object_or_404(PhysicalObjectType,
...
def collections_list(request):
    collections = []
    collections_qs = Collection.objects.order_by(
        'repository__repository_id', 'collection_id')
...
class DigitalObjectsCollectionListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DigitalObjectsCollectionListView,
                        self).get_context_data(**kwargs)
        temp = self.args[0]
        try:
        ...
        temp_coll_id = temp[3:]
        try:
            context['current_collection'] = Collection.objects.get(
                collection_id=temp_coll_id, repository=temp_repo)
...
class DigitalObjectDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DigitalObjectDetailView,
                        self).get_context_data(**kwargs)
...
class FestivalsListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        alldos = DigitalObject.objects.filter(
            published=True,
            related_festival__isnull=False,
            files__isnull=False,
            digi_object_format=imagetype)
        length = len(alldos) - 1
        count = 0
        ...
        if length > 0:
            while count < 3:
                num = randrange(0, length)
                if (alldos[num].files.count() > 0 and
                    alldos[num].files.all()[0]):
                    dos.append(alldos[num])
                    count += 1
...
class FestivalsAlphaListView(FestivalsListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(FestivalsAlphaListView,
                        self).get_context_data(**kwargs)
        if self.kwargs['alpha'] == '0':
            alpha = '#'
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        for festival_Occurrence_obj in context['festival_occurrences']:
            if festival_Occurrence_obj.has_images():
                fo_images = DigitalObject.objects.filter(
                    published=True,
                    related_festival=festival_Occurrence_obj,
                    files__isnull=False,
                    digi_object_format=imagetype).distinct()
...
class FestivalOccurrenceDetailView(DetailView):
    queryset = FestivalOccurrence.objects.filter(
...
class AwardsListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        objects_list = []
        imagetype = DigitalObjectType.objects.get(title='Image')
        alldos = DigitalObject.objects.filter(published=True,
                                              related_award__isnull=False,
                                              files__isnull=False,
                                              digi_object_format=imagetype)
        count = 0
        length = len(alldos) - 1
...
class AwardsAlphaListView(AwardsListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(AwardsAlphaListView,
                        self).get_context_data(**kwargs)
        if self.kwargs['alpha'] == '0':
            alpha = '#'
        else:
...
class AwardDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['award_candidates'] = AwardCandidate.objects.filter(
            award__id=context['award'].id).order_by('-year')
        ...
        for award_candidates_obj in context['award_candidates']:
            if award_candidates_obj.has_images():
                ac_images = DigitalObject.objects.filter(
                    published=True,
                    related_award=award_candidates_obj,
                    files__isnull=False,
                    digi_object_format=imagetype).distinct()
...
def search_view(request):
    query = creator_matches = \
            location_matches = \
            production_matches = \
            workrecord_matches = \
            festival_matches = \
            taggeditem_matches = \
            digitalobject_matches = False
...
def flatpage(request, url, **kwargs):
    ...
    if not url.startswith('/'):
        url = "/" + url
    f = get_object_or_404(TranslatingFlatPage,
                          url__exact=url,
...
def scalar_search_view(request):
    ...
    istream = urllib2.urlopen(
        urllib2.Request(url="http://localhost:8983/scalar/servlet?q="+query))
...
def search_do_view(request):
    ...
    return render_to_response('search/search_do.html',
                              context,
...
class TaggedItemsListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        result_list = []
        queryset = DigitalObject.objects.filter(
            tags__name=query, published=True)
        for item in queryset:
            result_list.append(item)
        if result_list:
