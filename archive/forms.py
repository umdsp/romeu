...
from archive.models import (Creator, Location, Stage, RelatedCreator,
                            WorkRecord, WorkRecordCreator,
                            Production, Role, DirectingMember, CastMember,
                            DesignMember, TechMember, ProductionMember,
                            AdvisoryMember, FestivalOccurrence,
                            FestivalParticipant, Repository, DigitalObject,
                            DigitalObject_Related_Creator,
                            AwardCandidate, RelatedWork, City)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            if self.instance.headquarter_city:
                self.initial['headquarter_city'] = \
                self.instance.headquarter_city.pk
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            if self.instance.second_creator:
                self.initial['second_creator'] = \
                self.instance.second_creator.pk
...
class DigitalObjectAdminForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            if self.instance.collection:
                self.initial['collection'] = \
                self.instance.collection.pk
            if self.instance.object_creator:
                self.initial['object_creator'] = \
                self.instance.object_creator.pk
            if self.instance.phys_obj_city:
                self.initial['phys_obj_city'] = \
                self.instance.phys_obj_city.pk
    ...
    class Meta(object):
        ...
        widgets = {
            ...
            'phys_obj_city':
                selectable_forms.AutoCompleteSelectWidget(
                    lookup_class=CityLookup, attrs={'size':'100'}), }
...
class DigitalObjectRelatedCreatorAdminForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(DigitalObjectRelatedCreatorAdminForm,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            if self.instance.festival_series:
                self.initial['festival_series'] = \
                self.instance.festival_series.pk
...
class AwardCandidateAdminForm(ModelForm):
    ...
    """
        model form for AwardCandidate class
    """

    def __init__(self, *args, **kwargs):

        """
            Award Candidate init method
        """
